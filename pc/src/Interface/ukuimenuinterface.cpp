...
QStringList UkuiMenuInterface::getDesktopFilePath()
{
    ...
    // QStringList inilist;
     for(int i=0;i<filePathList.size();++i)//将遍历得到的文件路径进行处理。
     {
          read_ini(filePathList.at(i));//读取配置文件
     }
...
void UkuiMenuInterface::read_ini(QString str_desktop)
{
...
//打开文件.desktop
              ...
              QByteArray fpbyte=str_desktop.toLocal8Bit();
              char* filepath=fpbyte.data();
              g_key_file_load_from_file(keyfile,filepath,flags,error);
             ...
             //关闭文件
              ...
              // settt->beginGroup("application");
               ...
               {                
                  filePathList.removeAll(str_desktop);
               return;
               }        
