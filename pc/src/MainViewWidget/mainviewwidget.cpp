...
void MainViewWidget::initUi()
{
    ...
    this->setWindowOpacity(0.45);//设置总体透明度//0.95-0.45
    this->setProperty("doNotBlur",true);
    QVBoxLayout* mainLayout=new QVBoxLayout;
    mainLayout->setContentsMargins(0,0,0,0);
    mainLayout->setSpacing(0);
    m_topWidget=new QWidget;
    m_topWidget->setStyleSheet("border:0px;background:transparent;");//transparent
  ...
  //  connect(m_commonUseWid,&CommonUseWidget::sendHideMainWindowSignal,this,&MainViewWidget::sendHideMainWindowSignal);
    ...
    QDBusConnection::systemBus().connect(QString(), QString("/com/ukui/desktop/software"), "com.ukui.desktop.software", "send_to_client", this, SLOT(client_get(QString)));
    connect(this, &MainViewWidget::UpdateSignal, [&](){        
...
void MainViewWidget::client_get(QString str)
{
    qDebug() << "Client get information from service!" << str;
    Q_EMIT UpdateSignal();
}
...
void MainViewWidget::addTopControl()
{
    ...
    char style[100];
    sprintf(style, "QLineEdit{border:0px;background-color:%s;border-radius:25px;}",QueryLineEditBackground);//  6-1
...
void MainViewWidget::initQueryLineEdit()
{
    ...
    m_queryWid->setStyleSheet("border:0px;background:transparent;border-radius:25px;");//搜索框样式，圆角
    ...
    m_queryText->setText(tr("Search Application"));//搜索栏中的文件，与translation中的对应，中英翻译
    m_queryText->setStyleSheet("background:transparent;color:#d9d9d9;border-radius:25px;");//border-radius:25px;
...
bool MainViewWidget::eventFilter(QObject *watched, QEvent *event)
{
    if(watched==m_queryLineEdit)
    {
        ...
        {
             char style[200];
             sprintf(style, "QLineEdit{border:1px solid %s;background-color:%s;border-radius:25px;color:#ffffff;}",
        ...
        else if(event->type()==QEvent::FocusOut)
        {
            ...
            if(m_queryLineEdit->text().isEmpty())
            {
                ...
                {
                    char style[100];
                    sprintf(style, "QLineEdit{border:0px;background-color:%s;border-radius:25px;}",QueryLineEditBackground);//
                    m_animation->stop();
                    m_queryLineEdit->setStyleSheet(style);
                    m_queryText->adjustSize();
                ...
                char style[100];
                sprintf(style, "QLineEdit{border:0px;background-color:%s;border-radius:25px;color:#ffffff;}",
...
void MainViewWidget::loadMaxMainView()
{
    ...
    m_topWidget->setFixedSize(this->width(),Style::TopWidgetHeight);//Style::widthavailable
...
//                                  0);
    m_topLayout->setContentsMargins((m_topWidget->width()-m_queryLineEdit->width())/2,
                                  0,
                                  (m_topWidget->width()-m_queryLineEdit->width())/2,
                                  0);
...
void MainViewWidget::widgetMakeZero()
{
 ...
 //   m_functionWid->widgetMakeZero();
   // m_fullFunctionWid->widgetMakeZero();
    ...
    char style[100];
    sprintf(style, "QLineEdit{border:0px;background-color:%s;border-radius:25px;}",QueryLineEditBackground);//2-25
    m_queryLineEdit->setStyleSheet(style);
...
void MainViewWidget::moveScrollBar(int type)
{
    ...
    {
        if(m_isFullScreen)
            m_fullSearchResultWid->moveScrollBar(type);// dxg
        else
