...
void MainWindow::initUi()
{
    ...
    this->setStyleSheet("border:0px;background:yellow;");//transparent
    m_frame=new QFrame;
...
void MainWindow::stateChangedSlot(QAbstractAnimation::State newState, QAbstractAnimation::State oldState)
{
    ...
    if(m_isFullScreen && newState==QAbstractAnimation::Stopped)
    {
      ...
      // // m_sideBarWid->loadMaxSidebar();//dxg
...
void MainWindow::loadMainWindow()
{
    ...
    else
    {
        if(position==0)
          ...
          m_mainViewWid->loadMaxMainView();//加载全屏窗口
          m_isFullScreen=true;
    }
    setFrameStyle();
...
void MainWindow::setFrameStyle()
{
    ...
    if(QGSettings::isSchemaInstalled(QString("org.ukui.control-center.personalise").toLocal8Bit()))
    {
        ...
        if(gsetting->keys().contains(QString("transparency")))
        {
            ...
            m_defaultBackground=QString("rgba(100, 100, 100, 0.45"+QString::number(transparency)+")");//修改背景透明度0.7-0.45 19,19,20-255,255,255
        }
        else
            m_defaultBackground=QString("rgba(100, 100, 100, 0.45)");
    }
    else
        m_defaultBackground=QString("rgba(100, 100, 100, 0.45)");
