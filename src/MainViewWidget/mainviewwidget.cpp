...
void MainViewWidget::add_top_control()
{
    ...
    minmaxLayout=new QHBoxLayout;
    minmaxbtn=new QToolButton;
...
void MainViewWidget::add_query_lineedit()
{
    ...
    querylineEdit->setPlaceholderText("搜索程序和文件");
...
bool MainViewWidget::eventFilter(QObject *watched, QEvent *event)
{
    if(watched==querylineEdit)
    {
        if(event->type()==QEvent::FocusIn)
        {
             ...
             QLayoutItem* child;
             if((child=mainLayout->takeAt(1))!=0)
             {
...
void MainViewWidget::set_max_btn()
{
    ...
    QPixmap pixmap(":/data/img/mainviewwidget/全屏.svg");
    minmaxbtn->setIconSize(pixmap.size());
    minmaxbtn->setIcon(QIcon(pixmap));
    minmaxbtn->setStyleSheet("QToolButton{background:transparent;border:0px;padding-left:0px;}\
...
void MainViewWidget::set_min_btn()
{
    ...
    QPixmap pixmap(":/data/img/mainviewwidget/还原.svg");
    minmaxbtn->setIconSize(pixmap.size());
    minmaxbtn->setIcon(QIcon(pixmap));
    minmaxbtn->setStyleSheet("QToolButton{background:transparent;border:0px;padding-left:0px;}\
                                 QToolButton:hover{background-color:#414e59;}\
